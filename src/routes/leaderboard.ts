...
router.get('/', validationMiddleware.validateLeaderboardQuery, async (req: Request, res: Response) => {
  try {
    ...
    const result = await playerService.getPlayersByWinCountPaginated(page, limit, filters);
...
router.get('/efficiency', validationMiddleware.validateLeaderboardQuery, async (req: Request, res: Response) => {
  try {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 10;
    ...
    const result = await playerService.getPlayersByEfficiencyPaginated(page, limit, filters);
