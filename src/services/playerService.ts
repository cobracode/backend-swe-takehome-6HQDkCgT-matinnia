...
export class PlayerService {
  ...
  async getPlayersByWinCountPaginated(
    ...
    console.log(
      `🏆 Getting paginated players by win count - page: ${page}, limit: ${limit}`
    )
    ...
    if (filters) {
      if (filters.minGames !== undefined) {
        players = players.filter(
          (p) => p.stats.gamesPlayed >= filters.minGames!
        )
      }
  ...
  async getPlayersByEfficiencyPaginated(
    ...
    console.log(
      `⚡ Getting paginated players by efficiency - page: ${page}, limit: ${limit}`
    )
  ...
  async updatePlayerStatsAfterGame(
    ...
    return this.playerModel.updatePlayerStatsAfterGame(
      playerId,
      gameResult,
      movesCount
    )
